...
with st.sidebar.form(key="my_form"):
    ...
    species_unique = collection.distinct(
        "specieName", {"specieName": {"$ne": None}}
    )
    species = st.multiselect("Species name", species_unique, help="Optional")

    recorded_by_unique = collection.distinct("recordedBy", {"recordedBy": {"$ne": None}})
...
if len(query_list) > 1:
    for document in cursor_list:
        if(document["decimalLatitude"]):
            latitude_location.append(float(document["decimalLatitude"].replace(",",".")))
            longitude_location.append(float(document["decimalLongitude"].replace(",",".")))   
...
if submit_button:  # or all_samples
    for document in cursor_list:
        ...
        if document["decimalLatitude"] and document["decimalLongitude"]:
